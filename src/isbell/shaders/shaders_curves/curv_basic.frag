...
//    float cosAlpha = abs(dot(viewDirection, reflectDirection));
    ...
    return
    texture(texture_ambient, v_uv) * intencities.x +
    texture(texture_diffuse, v_uv) * intencities.y * cosTheta * lightFactor(light, pos) +
    texture(texture_specular, v_uv) * intencities.z  * pow(cosAlpha, intencities.w) * lightFactor(light, pos);
}
