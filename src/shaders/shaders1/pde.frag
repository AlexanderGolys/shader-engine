...
vec4 colorFromLinearPointlight(mat4 light, vec3 camPos, vec3 pos, vec3 n, vec2 uv)
{
...
void main()
{
	...
	color = saturate(colorFactorFromSingleLight(light1, camPosition, v_position, normal, v_uv) +
    ...
    float collar = 1-smoothstep(0, .1, min(v_color.x - t, pow(v_color.x, 2)));
    color = mix( vec4(.3, .5, .9, 1.), color, collar);
    color.a = 1.;
}
