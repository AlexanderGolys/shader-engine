...
//	float cosAlpha = max(dot(viewDirection, reflectDirection), 0.);
    ...
    float c = smoothstep(0.0, 100.0, v_color.x);
    vec4 col = mix(vec4(.7, .75, .99, 1.0), vec4(.55, .5, .9, 1.0), c);
//
...
void main()
{
	...
	vec3 normal = normalize(v_position - v_color.yzw);
    ...
    color.a = 1.0; // Set alpha to 1 for full opacity
