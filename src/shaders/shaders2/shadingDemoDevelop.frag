...
//    }
	...
	color = colorFromPointlight(light1, camPosition, v_position, normal, v_uv*.98, 1) +
			        colorFromPointlight(light2, camPosition, v_position, normal, v_uv*1.01, 1) +
			        colorFromPointlight(light3, camPosition, v_position, normal, v_uv*1.06, 1);


//    color.rgb = pow(color.rgb, vec3(1.0/2.2));
//   float bd = smoothstep(-.08, .04, v_uv.x + v_uv.y - t);
//   float bd2 = smoothstep(.0, 0.04, v_uv.x + v_uv.y - t);
