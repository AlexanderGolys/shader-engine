...
//    float t = time*2.f;
	...
	color = saturate(saturate(colorFromPointlight(light1, camPosition, v_position, normal, v_uv.xx)) +
			saturate(colorFromPointlight(light2, camPosition, v_position, normal, v_uv.xx)) +
			saturate(colorFromPointlight(light3, camPosition, v_position, normal, v_uv.xx)));
//    float r_max = 0.9;
//    float red_x = smoothstep(1, 0.1, abs(v_position.x + 3.0))*r_max;
//    float red_t = smoothstep(.3, 0, time - v_color.w) * float(t > v_color.w);
//    color.r +=  red_x * red_t;
	color.a = 1;
